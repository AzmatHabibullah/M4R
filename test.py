...
from representation import polar_to_point, projection_onto_spin_half
import pytest
import numpy as np
from qutip import *
...
def test_m_conserved_for_c0_zero(c2, precision, psi0):
    ...
    x0, y0, z0 = polar_to_point(thetas[0], phis[0])
    normals0 = np.array([x0, y0, z0]).T

    normals0 = np.sort(normals0, axis=0)
    n0 = np.array([np.sum(normals0[2*i:2*i+2], axis=0) for i in range(int(F))])
    alpha0 = np.dot(normals0[0], normals0[1])
    for i, psi in enumerate(psi_evolution):
        ...
        if i > 0:
            ...
            normals = np.array([x, y, z]).T
            normals = np.sort(normals, axis=0)
            n = np.array([np.sum(normals[2*i:2*i+2], axis=0) for i in range(int(F))])
            n1 = np.array([x[0], y[0], z[0]])
            n2 = np.array([x[1], y[1], z[1]])
            assert np.linalg.norm(n - n0) < 10e-5                             # check n1 + n2 conserved
            alpha = np.dot(n1, n2)                                           # check angle between conserved
            assert np.linalg.norm(alpha - alpha0) < 10e-5


    # n1 + n2 conserved for F = 1 (check with more cases)
    # check what is conserved for F = 2. why is n1 + n2 conserved for F=1? what's significant about this?
