...
def plot(ic, precision=50, t1=1, draw_m=True, return_psi=False, d3=False, func=None, plot_p1=False):
    if func is None:
        func = c_nought_zero
    ...
    plot_evolution(psi, clear=True, plot_p1=plot_p1, d3=d3)
    if return_psi:
        return psi
...
def mean_field_RHS(F=1, c0=1, c2=1):
    def to_return(psi, m):
        ...
        m = Qobj(np.array([expect(jmat(F, "x"), p), expect(jmat(F, "y"), p),
                           expect(jmat(F, "z"), p)]))
        return c2*(m[0, 0]*jmat(F, "x") + m[1, 0]*jmat(F, "y") + m[2, 0]*jmat(F, "z"))*psi*1/1.0j + c0*1/1.0j*psi*np.abs(psi**2)
    return to_return
